    ...
    R: ReadAt,
{
    ...
    // PATCHED (see the notice at the top of lib.rs): upstream hard-rejects the
    // whole filesystem here if any incompatible-feature bit outside a small
    // allowlist is set - which rejected CSUM_SEED, a routine flag on modern
    // real-world ext4 filesystems, and would equally reject a filesystem for
    // merely *supporting* COMPRESSION/INLINE_DATA/ENCRYPT even if not one
    // actual file on it uses them. `IncompatibleFeature::from_bits` above
    // already rejected anything genuinely unrecognized. Per-inode correctness
    // for data-affecting features (COMPRESSION, INLINE_DATA, ENCRYPT) is
    // still enforced individually by `Inode::only_relevant_flag_is_extents`
    // when that specific inode's data or directory listing is read, so this
    // only widens what CAN be opened, not what is silently misread.
